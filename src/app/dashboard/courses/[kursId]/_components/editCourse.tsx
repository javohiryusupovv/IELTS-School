...
export default function EditCourse({ course }: Props) {
    ...
    const totalValue = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try{
            ...
            const updatePromise = updateCourseServer(course._id, updatedCourse, pathname);
                    ...
                    <form onSubmit={totalValue} className="w-full mb-5">
                        ...
                        >
                            ...
                            <p className="relative top-0 left-0 group py-2 px-3 rounded-md bg-gray-100 cursor-not-allowed">
                                {typeof course.teacher === 'object' ? `${course.teacher.teacherName} ${course.teacher.teacherSurname}` : null}
                                <span className="opacity-0 group-hover:opacity-100 group-hover:translate-y-2 transition-all duration-300 absolute left-0 -bottom-5 px-2 bg-red-500 text-white rounded text-[12px]">O'qituvchini o'zgartirish mumkin emas !</span>
                            </p>
                        </label>
                        <SheetClose asChild>
                            <button
