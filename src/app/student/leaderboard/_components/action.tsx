...
const medalColors = [
    {
      ...
      icon: <GiCrown className="leader absolute right-5 -top-[34px] w-10 h-10 text-[#FFD700]" />,
    },
    {
      bg: "bg-gray-400/80",
      ...
      icon: <PiCrownLight className="absolute left-0 -top-[30px] -rotate-[20deg] text-[#C0C0C0] w-10 h-10" />,
    },
    {
      bg: "bg-orange-300/80",
      ...
      icon: <PiCrownLight className="absolute right-0 -top-[30px] rotate-[20deg] text-[#CD7F32] w-10 h-10" />,
    }
  ];
                ...
                <div className="flex justify-between gap-2 w-full mb-4 h-[230px] container-leaderboard">
                    {[1, 0, 2].map((pos, i) => {
                        ...
                        return (
                            <div
                                key={i}
                                className={`leader-card flex justify-center items-center relative ${pos === 0 ? "top-10" : "top-20"} left-0 w-[130px] h-[150px] ${styles.bg} rounded-t-3xl`}
                            >
                                <article className={`absolute z-[1] -top-10 ${pos === 2 ? "right-6" : pos === 0 ? "left-[20%]" : "left-6"} w-20 h-20 border-2 rounded-full ${styles.border}`}>
                                    {styles.icon}
                                    <Image
                                        ...
                                        alt="LeaderBoard Img"
                                    />
                                    <span className={`flex justify-center items-center absolute left-[30px] -bottom-2 z-[2] w-[20px] h-[20px] rounded-full text-white text-[12px] font-semibold ${styles.badgeColor}`}>
                                        {pos + 1}
                                    </span>
                                </article>
                    ...
                    {studentsSort.slice(3, 10).map((student, index: number) => (
                        ...
                        >
                            <article className="flex items-center gap-5 ">
                                <span className="text-[18px] font-bold text-orange-500">{index + 4}</span>
